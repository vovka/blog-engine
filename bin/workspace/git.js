...
export const repositoryName = repository => repository
  .replace(/^git@[^:]+:/, '')
  .replace(/^https?:\/\/[^/]+\//, '')
  .replace(/\.git$/, '');
...
const validateCheckout = (source, entry) => {
  ...
  if (repositoryName(readRemote(source)) !== repositoryName(entry.repository)) {
...
const verifyEnhancerAccess = entry => {
  ...
  run('gh', ['api', `repos/${repositoryName(entry.repository)}`, '--silent'], { env, capture: true });
  return env;
};

const clonePrivate = (entry, target) => {
  const env = verifyEnhancerAccess(entry);
  run('gh', ['repo', 'clone', repositoryName(entry.repository), target, '--', '--no-checkout'], { env });
